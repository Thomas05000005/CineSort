...
class TierDistributionRealisticTests(unittest.TestCase):
    ...
    def test_panel_of_10_movies_produces_varied_distribution(self) -> None:
        ...
        self.assertLess(dist.get("Reject", 0), 9, f"Trop de Reject : {dict(dist)}. Bug initial : tout-en-Reject.")
        ...
        self.assertGreaterEqual(healthy, 1, f"Aucun film classe Silver+ : {dict(dist)}. Les seuils sont trop hauts.")
    ...
    def test_uhd_remux_premium_reaches_top_tier(self) -> None:
        ...
        self.assertIn(
            res["tier"],
            ("Platinum", "Gold"),
            f"UHD Remux premium devrait etre Platinum/Gold, obtenu {res['tier']} (score={res['score']}).",
        )
    ...
    def test_1080p_bluray_clean_reaches_at_least_silver(self) -> None:
        ...
        self.assertIn(
            ...
            f"1080p BluRay propre devrait etre Bronze+ : tier={res['tier']}, score={res['score']}.",
        )
        self.assertGreaterEqual(res["score"], 25, f"1080p BluRay propre devrait scorer >= 25 : score={res['score']}.")
...
class BronzeBiasRecalibrationTests(unittest.TestCase):
    ...
    def test_1080p_avc_8mbps_5_1_reaches_bronze(self) -> None:
        ...
        self.assertGreaterEqual(res["score"], 40, f"1080p AVC 8Mbps 5.1 doit atteindre Bronze : score={res['score']}")
        self.assertIn(
            res["tier"],
    ...
    def test_simulated_library_50_1080p_distribution_realistic(self) -> None:
        ...
        self.assertGreaterEqual(
            len(dist),
            2,
            f"Bibliotheque 1080p : distribution trop uniforme {dict(dist)}, au moins 2 tiers differents attendus.",
        )
        silver_or_better = dist.get("Silver", 0) + dist.get("Gold", 0) + dist.get("Platinum", 0)
        self.assertGreaterEqual(
            silver_or_better, 1, f"Aucun film classe Silver+ : {dict(dist)}. Les seuils sont mal calibres."
        )
        # Verifie aussi qu'on n'a pas tout-en-un-tier (mutation simple).
        bronze_or_below = dist.get("Bronze", 0) + dist.get("Reject", 0)
        self.assertLess(
            bronze_or_below, len(tiers), f"Tous les films classes Bronze/Reject -> bug recalibration : {dict(dist)}"
        )

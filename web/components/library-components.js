/* components/library-components.js — v7.6.0 Vague 3
 * Composants Library / Explorateur :
 *   - FilterSidebar : filtres composables (10 dimensions)
 *   - LibraryTable  : table v5 avec mini score circle + warnings badges
 *   - PosterGrid    : grid view (posters)
 *   - SmartPlaylists: sidebar des playlists
 *
 * API publique :
 *   window.LibraryComponents.renderFilterSidebar(container, opts)
 *   window.LibraryComponents.renderLibraryTable(container, rows, opts)
 *   window.LibraryComponents.renderPosterGrid(container, rows, opts)
 *   window.LibraryComponents.renderSmartPlaylists(container, playlists, opts)
 */
(function () {
  "use strict";
  ...
  /* ===========================================================
   * Dimensions de filtres (10) — meme nomenclature que le backend
   * =========================================================== */
   ...
   * }} opts
   */
  function renderFilterSidebar(container, opts) {
    if (!container) return;
    opts = opts || {};
    ...
    html += `
      <div class="v5-filter-section">
        <label class="v5-filter-section-title" for="v5-filter-search">Recherche</label>
        <input type="text" id="v5-filter-search" class="v5-input" data-filter-key="search"
               placeholder="Titre du film..."
               value="${_esc(active.search || "")}">
      </div>
    `;

    // Dimensions categorielle
    Object.entries(FILTER_DIMENSIONS).forEach(([key, dim]) => {
      ...
      const chips = dim.options.map((opt) => {
        ...
        return `
          <button type="button"
                  class="v5-filter-chip ${isActive ? "is-active" : ""}"
                  data-filter-key="${_esc(key)}"
                  data-filter-value="${_esc(opt.value)}"
                  aria-pressed="${isActive ? "true" : "false"}">
            ${_esc(opt.label)}${count}
          </button>
        `;
      }).join("");
      html += `
        <div class="v5-filter-section">
          <label class="v5-filter-section-title">${_esc(dim.label)}</label>
    ...
    html += `
      <div class="v5-filter-section">
        <label class="v5-filter-section-title">Année</label>
    ...
    html += `
      <div class="v5-filter-footer">
        <button type="button" class="v5-btn v5-btn--sm v5-btn--ghost" data-v5-filter-reset>
          Réinitialiser
        </button>
      </div>
    </div>
    ...
    const _emitChange = () => {
      ...
      debounceTimer = setTimeout(() => {
        const newFilters = _readFilters(container);
        if (typeof opts.onChange === "function") opts.onChange(newFilters);
      }, 150);
    };
    ...
    container.querySelectorAll("input[data-filter-key]").forEach((inp) => {
      inp.addEventListener("input", _emitChange);
    });
  ...
  function _readFilters(root) {
    const out = {};
    // Chips
    const chips = root.querySelectorAll(".v5-filter-chip.is-active");
    chips.forEach((c) => {
   ...
   * @param {HTMLElement} container
   * @param {Array} rows
   * @param {{ onRowClick?, sort?, onSortChange? }} opts
   */
  function renderLibraryTable(container, rows, opts) {
    if (!container) return;
    opts = opts || {};
    ...
    const sortableCols = ["title", "score", "year", "duration"];
    ...
    rows.forEach((r) => {
      ...
      const scoreHtml = scoreVal != null
        ? `<span class="v5-library-score-mini v5-library-score-mini--${_esc(tier)}">${scoreVal}</span>`
        : `<span class="v5u-text-muted">—</span>`;
      const tierBadge = `<span class="v5-badge v5-badge--tier-${_esc(tier)}">${_esc(tier)}</span>`;
      const dur = r.duration_min ? `${r.duration_min} min` : "—";
      const warnings = (r.warnings || []).slice(0, 3).map((w) =>
        `<span class="v5-badge v5-badge--severity-warning v5-library-warn" title="${_esc(w)}">${_esc(_shortWarnLabel(w))}</span>`
      ).join("");
      ...
      html += `
        <tr class="row-tier-${_esc(tier)}" data-row-id="${_esc(r.row_id)}" tabindex="0">
          <td class="v5-library-title">${_esc(r.title || "—")}</td>
          <td class="v5u-tabular-nums">${scoreHtml}</td>
          <td>${tierBadge}</td>
          <td>${_esc(r.resolution || "—")}</td>
          <td>${_esc(String(r.codec || "—").toUpperCase())}</td>
          <td>${_esc(String(r.hdr || "—").toUpperCase())}</td>
          <td class="v5u-tabular-nums">${_esc(r.year || "—")}</td>
          <td class="v5u-tabular-nums">${_esc(dur)}</td>
    ...
    container.querySelectorAll("tr[data-row-id]").forEach((tr) => {
      tr.addEventListener("click", () => {
        ...
        else if (typeof window.navigateTo === "function") window.navigateTo("film/" + rowId);
      });
    ...
    container.querySelectorAll("th.sortable").forEach((th) => {
      th.addEventListener("click", () => {
        ...
        const newSort = _nextSort(currentSort, key);
        opts.onSortChange(newSort);
      });
    });
  }
  ...
  const _WARN_SHORT = {
    dv_profile_5: "DV5",
    hdr_metadata_missing: "HDR?",
    runtime_mismatch: "Runtime",
    short_file: "Court",
    low_confidence: "Confidence",
    category_imbalance: "V/A delta",
    fake_lossless: "Faux LL",
    dnr_partial: "DNR",
    fake_4k_confirmed: "Faux 4K",
  };

  function _shortWarnLabel(w) {
    return _WARN_SHORT[w] || w.substr(0, 8);
  }

  /* ===========================================================
   * POSTER GRID (vue grille)
   * =========================================================== */

  /**
   * @param {HTMLElement} container
   * @param {Array} rows
   * @param {{ onRowClick?: function }} opts
   */
  function renderPosterGrid(container, rows, opts) {
    if (!container) return;
    opts = opts || {};
    if (!Array.isArray(rows) || rows.length === 0) {
      container.innerHTML = `<div class="v5-library-empty">Aucun film ne correspond aux filtres.</div>`;
    ...
    const cards = rows.map((r, idx) => {
      ...
      const posterStyle = r.poster_url ? `background-image: url('${_esc(r.poster_url)}')` : "";
      return `
        <article class="v5-poster-card v5-poster-card--library stagger-item"
                 role="listitem" style="--order: ${Math.min(idx, 20)}"
                 data-row-id="${_esc(r.row_id)}" tabindex="0"
                 aria-label="${_esc(r.title || "Film")}, score ${_esc(score)}">
          <div class="v5-poster-image" style="${posterStyle}">
            ${!r.poster_url ? `<div class="v5-poster-placeholder">${_svg('<rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/>', 32)}</div>` : ""}
            <div class="v5-poster-score-overlay v5-poster-score-overlay--${_esc(tier)}">${_esc(score)}</div>
          </div>
          <div class="v5-poster-meta">
            <div class="v5-poster-title v5u-truncate">${_esc(r.title || "?")}</div>
            <div class="v5-poster-sub v5u-text-muted">${_esc(r.year || "")} · ${_esc(String(r.resolution || "").toUpperCase())}</div>
          </div>
        </article>
      `;
    }).join("");
  ...
  /**
   * @param {HTMLElement} container
   * @param {Array<{id, name, filters, preset?}>} playlists
   * @param {{ activeId?, onSelect?, onSaveCurrent?, onDelete? }} opts
   */
  function renderSmartPlaylists(container, playlists, opts) {
    if (!container) return;
    opts = opts || {};
    ...
    const items = playlists.map((p) => {
      ...
      const deleteBtn = !isPreset
        ? `<button type="button" class="v5-btn v5-btn--sm v5-btn--ghost v5-btn--icon"
                   data-playlist-delete="${_esc(p.id)}"
                   aria-label="Supprimer">
             ${_svg('<polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>', 12)}
           </button>`
        : "";
      return `
        <div class="v5-playlist-item ${isActive ? "is-active" : ""} ${isPreset ? "is-preset" : ""}"
             data-playlist-id="${_esc(p.id)}">
          <button type="button" class="v5-playlist-name" data-playlist-select="${_esc(p.id)}">
            ${_esc(p.name)}
    ...
    container.querySelectorAll("[data-playlist-delete]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const id = btn.dataset.playlistDelete;

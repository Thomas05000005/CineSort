/* dashboard/components/home-widgets.js — v7.6.0 Vague 2 (ES module)
...
function _icon(name, size) { return _svg(ICONS[name] || ICONS["alert-circle"], size || 18); }
...
function _renderTrend(trend) {
  ...
  if (val.startsWith("↑") || val.startsWith("+") || /^trend_up/.test(val)) {
    cls = "v5-kpi-trend v5-kpi-trend--up"; icon = _icon("trend-up", 12);
  } else if (val.startsWith("↓") || val.startsWith("-") || /^trend_down/.test(val)) {
    cls = "v5-kpi-trend v5-kpi-trend--down"; icon = _icon("trend-down", 12);
  }
  return `<span class="${cls}">${icon}<span>${escapeHtml(val)}</span></span>`;
}

export function renderKpiGrid(container, kpis) {
  if (!container) return;
  if (!Array.isArray(kpis) || kpis.length === 0) { container.innerHTML = ""; return; }
  let html = '<div class="v5-kpi-grid" role="list">';
  kpis.forEach((k, idx) => {
    const trendHtml = _renderTrend(k.trend);
    const tierClass = k.tier ? `v5-kpi-card--tier-${escapeHtml(k.tier)}` : "";
    const suffix = k.suffix ? `<span class="v5-kpi-suffix">${escapeHtml(k.suffix)}</span>` : "";
    ...
    html += `
      <article class="v5-kpi-card ${tierClass} stagger-item" role="listitem" style="--order: ${idx}">
        <header class="v5-kpi-header">${iconHtml}<span class="v5-kpi-label">${escapeHtml(k.label)}</span></header>
        <div class="v5-kpi-body">
          <span class="v5-kpi-value v5u-tabular-nums">${escapeHtml(k.value)}</span>${suffix}${trendHtml}
        </div>
      </article>`;
  });
  html += "</div>";
...
export function renderInsights(container, insights, onAction) {
  if (!container) return;
  if (!Array.isArray(insights) || insights.length === 0) {
    container.innerHTML = `<div class="v5-insight-empty">Aucune alerte. Tout va bien.</div>`;
    return;
  }
  const cards = insights.map((it, idx) => {
    const sev = String(it.severity || "info").toLowerCase();
    const icon = _icon(it.icon || "alert-circle", 16);
    const canAction = !!it.filter_hint;
    return `
      <article class="v5-insight-card v5-insight-card--${escapeHtml(sev)} stagger-item"
               role="listitem" style="--order: ${idx}" data-insight-type="${escapeHtml(it.type)}">
        <span class="v5-insight-icon">${icon}</span>
        <span class="v5-insight-label">${escapeHtml(it.label)}</span>
        <div class="v5-insight-actions">
          ...
          <button type="button" class="v5-btn v5-btn--sm v5-btn--ghost v5-btn--icon"
                  data-insight-dismiss="${idx}" aria-label="Masquer">${_icon("x", 14)}</button>
        </div>
      </article>`;
  }).join("");
...
export function renderPosterCarousel(container, items, opts = {}) {
  if (!container) return;
    ...
    return `
      <article class="v5-poster-card stagger-item" role="listitem" style="--order: ${idx}"
        ...
        <div class="v5-poster-meta">
          ...
          <div class="v5-poster-score"><span class="v5-badge v5-badge--tier-${escapeHtml(tier)}">${escapeHtml(score)}</span></div>
        </div>
      </article>`;
  }).join("");
  container.innerHTML = `<div class="v5-poster-carousel v5-scroll" role="list">${cards}</div>`;
  const onClickFilm = typeof opts.onClickFilm === "function" ? opts.onClickFilm : null;
  container.querySelectorAll("[data-poster-film]").forEach((card) => {
    card.addEventListener("click", () => {
      const rowId = card.dataset.posterFilm;
      if (onClickFilm) onClickFilm(rowId);

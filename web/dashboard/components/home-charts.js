/* dashboard/components/home-charts.js — v7.6.0 Vague 2 (ES module)
 * Parite avec web/components/home-charts.js desktop.
 */
...
export function renderDonut(container, data) {
  if (!container) return;
  if (!data || !data.counts) { container.innerHTML = `<div class="v5-chart-empty">Aucune donnée.</div>`; return; }
  const counts = data.counts;
  const total = Number(data.scored_total || data.total || 0);
  if (total === 0) { container.innerHTML = `<div class="v5-chart-empty">Aucun film classe.</div>`; return; }
  ...
  for (const tier of order) {
    ...
    arcs.push({ tier, d, color: TIER_COLORS[tier], pct: Number(data.percentages?.[tier] || 0), count });
    cumulativeAngle = end;
  }

  const arcsSvg = arcs.map((a) => `
    <path class="v5-donut-arc" d="${a.d}"
          stroke="${a.color}" stroke-width="${stroke}" fill="none" stroke-linecap="butt"
          data-tier="${escapeHtml(a.tier)}" data-count="${a.count}" data-pct="${a.pct}">
      <title>${escapeHtml(TIER_LABELS[a.tier])} : ${a.count} (${a.pct.toFixed(1)}%)</title>
    </path>`).join("");

  const legend = arcs.map((a) => `
    <li class="v5-donut-legend-item">
      <span class="v5-donut-legend-swatch" style="background: ${a.color}"></span>
      <span class="v5-donut-legend-label">${escapeHtml(TIER_LABELS[a.tier])}</span>
...
export function renderLine(container, points) {
  if (!container) return;
  if (!Array.isArray(points) || points.length === 0) {
    container.innerHTML = `<div class="v5-chart-empty">Pas assez de donnees.</div>`; return;
  }
  ...
  if (scored.length < 2) {
    container.innerHTML = `<div class="v5-chart-empty">Trop peu de points (${scored.length}).</div>`; return;
  }
  ...
  coords.forEach((c) => {
    ...
    if (!started) { path += `M ${c.x.toFixed(2)} ${c.y.toFixed(2)}`; started = true; }
    else { path += ` L ${c.x.toFixed(2)} ${c.y.toFixed(2)}`; }
  });
  ...
  if (validCoords.length >= 2) {
    const yBase = h - padY;
    fillPath = `M ${validCoords[0].x.toFixed(2)} ${yBase.toFixed(2)}`;
    validCoords.forEach((c) => { fillPath += ` L ${c.x.toFixed(2)} ${c.y.toFixed(2)}`; });
    fillPath += ` L ${validCoords[validCoords.length - 1].x.toFixed(2)} ${yBase.toFixed(2)} Z`;
  }

  const dots = validCoords.map((c) => `
    <circle class="v5-line-dot" cx="${c.x.toFixed(2)}" cy="${c.y.toFixed(2)}" r="3"
            data-date="${escapeHtml(c.date)}" data-score="${c.score}">
      <title>${escapeHtml(c.date)} : score ${c.score} (${c.count} films)</title>
    </circle>`).join("");
  ...
  const firstScored = scored[0], lastScored = scored[scored.length - 1];
  ...
  container.innerHTML = `
    <div class="v5-line-wrap">
      ...
      <svg class="v5-line-svg" viewBox="0 0 ${w} ${h}" width="100%" height="${h}"
        ...
        <defs>
          <linearGradient id="v5LineGradientDash" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stop-color="var(--accent)" stop-opacity="0.35"/>
            <stop offset="100%" stop-color="var(--accent)" stop-opacity="0"/>
          </linearGradient>
        </defs>
        ${yTicks}
        <path class="v5-line-fill" d="${fillPath}" fill="url(#v5LineGradientDash)"/>
        <path class="v5-line-path" d="${path}" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ${dots}
      </svg>
      <div class="v5-line-footer"><span>${escapeHtml(first)}</span><span>${escapeHtml(last)}</span></div>
    </div>`;
}

...
def _vobsub_paired_indexes(subtitles: List[SubtitleInfo]) -> Set[str]:
    """Noms des `.idx` qui forment une paire VobSub avec un `.sub` de meme stem.

    Fix audit 2026-07-15 (#749) : la paire compte pour UN sous-titre dans le
    decompte des doublons de langue, sinon `Film.fr.idx` + `Film.fr.sub` levent
    un faux `subtitle_duplicate_lang` sur toute bibliotheque de DVD remuxes.

    L'appartenance a une paire est verifiee par la PRESENCE du `.sub` frere, et
    non deduite de la seule extension `.idx`. Deux raisons :

    * un `.idx` ORPHELIN n'est pas une paire — c'est un fichier a part entiere,
      qui a sa propre langue et doit donc rester dans le decompte. L'ecarter sur
      la seule extension masquerait un VRAI doublon (`Film.fr.idx` seul a cote
      de `Film.fr.srt`), c.-a-d. ferait taire une alerte : le mauvais sens ;
    * aucune autre couche ne materialise la paire comme un objet. Cote apply,
      `domain.core.classify_sidecars` rattache les sidecars a la video UN PAR UN
      par leur stem, et les tables d'extensions (`SIDECAR_METADATA_EXTS`,
      `RESIDUAL_SUBTITLE_EXTS`) listent `.idx` et `.sub` comme deux entrees
      independantes — l'orphelin y est traite exactement comme les autres. Le
      seul endroit ou les deux fichiers doivent redevenir UN, c'est le decompte
      des doublons de langue : cette fonction, et elle seule.
    """
...
def build_subtitle_report(
    ...
    *,
    ...
    #     que les sous-titres SANS tag de variante : deux `.fr.srt` restent
    #     impossibles (meme nom), deux variantes taguees ne sont pas un doublon
    #     utilisateur.
    #
    # (2) Fix audit 2026-07-15 (subtitle-vobsub, #749) : une paire VobSub
    #     (`Film.fr.idx` + `Film.fr.sub`, meme stem) est UN seul sous-titre
    #     stocke en deux fichiers. Elle ne porte AUCUN tag de variante, donc
    #     l'exclusion (1) ne la voit pas et 'fr' etait compte 2 fois.
    #     L'index reste present dans `count`, `formats` et `languages` — ce sont
    #     des mesures de FICHIERS ; seul le decompte des doublons l'ecarte.
    vobsub_indexes = _vobsub_paired_indexes(matched)
    lang_counts: Dict[str, int] = {}
    for s in matched:
        if not s.language:
            continue
        if s.filename in vobsub_indexes:
            continue

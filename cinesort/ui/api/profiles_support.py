...
Trois endpoints qui surcouchent l'existant `quality_profile_support` pour offrir
une vue homogene "liste de profils" (presets predefinis + profils custom user)
attendue par l'ecran Parametres.

Endpoints :
- `get_profiles(api)`               -> liste fusionnee {presets + custom} avec flag is_active
- `save_profile(api, profile)`       -> persiste un profil custom dans settings + validation tiers + poids
- `set_active_profile(api, id)`      -> active un profil (preset ou custom) dans settings + DB

Stockage des profils custom :
- `settings.custom_quality_profiles` : liste de profils JSON valides
- `settings.active_quality_profile_id` : id du profil actif (preset OU custom)
    ...
    Format reponse :
    {
      ...
      "profiles": [
        {id, preset_id, name, label, description, version, is_active, is_custom,
         tiers, weights, toggles, profile_json},
        ...
      ],
      "active_profile_id": "...",
    }

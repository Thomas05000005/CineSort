...
def run():
    ...
    st.apply.append_apply_operation(batch_id=bid, op_index=1, op_type="MOVE_FILE",
                                    src_path=str(src), dst_path=str(dst), reversible=True)
    ...
    raised = None
    try:
        st3.apply.close_apply_batch(batch_id=bid3, status="ROLLED_BACK_BY_ATOMIC",
                                    summary={"rollback_status": "ROLLED_BACK_BY_ATOMIC"})
    except Exception as e:  # ApplyBatchStateError (RuntimeError) AVANT le fix
        raised = type(e).__name__
    # relire le statut
    hist = st3.apply.list_apply_history(run_id="r3") if hasattr(st3.apply, "list_apply_history") else []
    final_status = None
    ...
    lib = tmp4 / "lib"; lib.mkdir(parents=True)
    ...
    op = {"id": 1, "op_index": 1, "op_type": "MOVE_FILE",
          "src_path": str(target), "dst_path": str(current), "reversible": 1, "undo_status": "PENDING"}
    st4.apply.append_apply_operation(batch_id=bid4, op_index=1, op_type="MOVE_FILE",
                                     src_path=str(target), dst_path=str(current), reversible=True)
    ...
    rep = _execute_undo_ops(_Api(), [op], st4, lambda lv, m: None, run_paths,
                            empty_bucket=None, residual_bucket=None, atomic=False)
    ...
    in_conflicts = list((run_paths.run_dir / "_review" / "_undo_conflicts").rglob("*.mkv")) \
        if (run_paths.run_dir / "_review" / "_undo_conflicts").exists() else []
    if case_insensitive:

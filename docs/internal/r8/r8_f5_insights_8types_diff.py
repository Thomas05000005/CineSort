...
FRONT_METIER = {"quality_reject","duplicates_probable","films_not_identified","subs_missing_fr",
                "sagas_incomplete","films_low_confidence","health_low","omdb_disagreements"}

class FakePerc:
    def list_perceptual_reports(self, run_id=None): return [{"global_tier_v2":"reject"}]
    def count_v2_warnings_flag(self, flag=None, run_ids=None): return 0
    def count_v2_tier_since(self, tier=None, since_ts=None): return 0
class FakeRun:
    def list_runs(self, limit=1): return []
class FakeStore:
    perceptual=FakePerc(); run=FakeRun()

librarian = {"health_score":35,"low_confidence_count":4,"suggestions":[

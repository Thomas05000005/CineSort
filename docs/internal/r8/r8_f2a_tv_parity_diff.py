...
def run():
    ...
    def rec2(op):
        ops2.append(dict(op) if isinstance(op, dict) else {"raw": str(op)})

    apply_tv_episode(
        cfg2, folder2, _row(), dry_run=False, log=(lambda lv, m: None), res=res2, record_op=rec2, **_roots(root2)
    )
    ...
    def rec3(op):
        ops3.append(dict(op) if isinstance(op, dict) else {"raw": str(op)})
    ...
    try:
        apply_tv_episode(
            cfg3, folder3, _row(), dry_run=False, log=(lambda lv, m: None), res=res3, record_op=rec3, **_roots(root3)
        )
    ...
    def rec4(op):
        ops4.append(dict(op) if isinstance(op, dict) else {"raw": str(op)})

    apply_tv_episode(
        cfg4, folder4, _row(), dry_run=True, log=(lambda lv, m: None), res=res4, record_op=rec4, **_roots(root4)
    )
    ...
    row5 = Row(
        video=f"{SRC_STEM}.mkv",
        proposed_title="Wrong",
        proposed_year=2019,
        tv_season=1,
        tv_episode=1,
        tv_episode_title="Pilot",
        tv_series_name="Wrong Series",
    )
    apply_tv_episode(
        cfg5,
        folder5,

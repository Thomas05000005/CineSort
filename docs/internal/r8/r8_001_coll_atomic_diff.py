...
def run():
    ...
    item_complete = video_moved_now and any(s.endswith(".srt") for s in sidecars_in_sub_after2) and any(s.endswith(".nfo") for s in sidecars_in_sub_after2)
    src_empty_now = not any(folder.glob(f"{stem}.*"))

    print("=== R8-001 DIFFERENTIEL (apply_collection_item) ===")
    print("--- BASELINE (casse, fige v9_coll_atomic_repro.out.txt) : half_applied=true, dedup_poisoned=true, no_rollback=true ---")
    print()
    ...
    print(f"VERDICT : {'CORRIGE (A atomicite + B retry re-traite, differentiel casse->correct prouve)' if fixed else 'NON corrige'}")

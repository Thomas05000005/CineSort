...
## ═══ SQUASH-MERGE loop → main (ÉTAPE 1 : résolution + diff, AVANT push) — 2026-06-19 ═══

Squash-merge des 427 commits R8 dans v1.5.2 (main 0882c50), sur branche de travail `merge/r8-into-main`.
Commit squash **52694db** (parent unique 0882c50 = vrai squash, pas de parent loop). **AUCUN push.**

### Écart v1.5.2 (0882c50) vs loop
1 commit : `feat(v152): tools UI + update flow + 5 vagues post-audit` — +1392 l. sur 16 fichiers
(tools_manager.py +143 backend, parametres.js tools UI bulk, accueil/doublons/traitement/film-detail).
Le backend (tools_manager.py, runtime_facade) **auto-mergé** (préservé côté HEAD).

### Verdict vues mortes (R8 supprime, vérifié contre v1.5.2)
Toutes MORTES dans v1.5.2 aussi (app.js redirige /qij,/library,/quality ; cluster d'auto-imports
qij/quality/quality-simulator/custom-rules + library/ entre eux ; 0 usage vivant). → **supprimées**
(library/ ×6, déjà supprimées qij/quality/etc. en R8). v1.5.2 les touchait sans les utiliser.

### 9 conflits résolus (R8 ⊕ v1.5.2 préservés)
| Fichier | Nature | Résolution |
|---|---|---|
| library/lib-apply/duplicates/validation.js | modify/delete | **R8 supprime** (vérifié mortes) |
| cinesort_api.py | 1 ligne | **R8** `_get_state_dir()` (refactor 29 usages) ; updater v1.5.2 préservé |
| traitement.js | polling | **R8** `_scheduleNextPoll` backoff ; auto-transition v1.5.2 préservée dans le handler R8 (plus fine) |
| accueil.js (×2) | TMDb alert + payload | **R8** fix R6-G (`_has_tmdb_api_key`) + extraction défensive ; updateInfo v1.5.2 gardé |
| doublons.js (×2) | cache + garde Apply | **R8** `_loadGroups(true)` R6-C + garde dangerConfirmModal (sur-ensemble) |
| film-detail.js (×3) | poster + messages | **R8** proxy+onerror R6-H, message R7-17 (additif), toast générique (console garde l'erreur) |
| parametres.js (×14) | tools/config UI | **R8 entier** = sur-ensemble fonctionnel (bulk tools auto_install/recheck/update, save_profile, recompute_all_scores TOUS préservés + config workers/pragma R8 + 6 toggles fantômes retirés) |

### Poids de la branche (léger, zéro bloat)
852 fichiers, +248114/-11235. **0 ajout** d'observe/dist_backup/test_library/EXE (le vrai bloat 325Mo
n'entre PAS). FAIL_*.png = 26 **suppressions**. .gitignore F6-b inclus (anti-bloat futur).
⚠️ **48 % du diff = 12 `suite_*.txt`** (118541 l. = logs pytest bruts dans docs/internal/r8) — régénérables,
discutables en public. Décision Thomas : les garder (preuves) ou les exclure avant push (git rm --cached, rapide).

### Sanity
Merge fonctionnel : **367 passed** (tests v1.5.2 probe_tools_manager + undo_24h OK = backend tools préservé ;
zones R8 OK). 1 échec = test_scope_apparence (PRÉ-EXISTANT baseline F3, indépendant du merge).

### État (AUCUN push)
main distant `0882c50` INTACT, loop `42875eb` INTACTE, f493abdc INTACT, backups `backup/pre-squash-main`
(main) + `backup/loop-pre-purge-2026-06-19` (loop). Push main = ÉTAPE 2 sur validation Thomas du diff.

---

## ═══ SQUASH-MERGE loop → main (ÉTAPE 2 : retrait logs bruts + push) — 2026-06-19 ═══

Sur décision Thomas : retrait des 12 `docs/internal/r8/suite_*.txt` (logs pytest bruts, régénérables,
~118k lignes = 48% du squash, ni preuve ni rejouable) AVANT push. `git rm --cached` (disque intact) +
`docs/internal/r8/suite_*.txt` ajouté au .gitignore. Squash RECRÉÉ en 1 commit propre (reset --soft +
recommit) pour que les blobs txt N'ENTRENT JAMAIS dans l'historique de main (pas seulement le tree final).

GARDÉS suivis (vraies preuves R8) : R8_CORRECTIONS.md (ce journal), scripts rejouables `r8_*_diff.py`,
BASELINE_R8.md (registre), F6B_GIT_ANALYSIS.md, captures `*.out.txt`. On a enlevé le BRUIT, pas la substance.

Push : main avancé en **fast-forward NORMAL** (jamais --force) depuis origin/main 0882c50 (vérifié inchangé
avant push). main = corrections R8 (F1→F6) + v1.5.2, légère, zéro bloat. loop conservée (archive du détail
des 427 commits). f493abdc intact.

...
def run():
    ...
    out["T4_ordre_ascendant"] = {"versions": versions, "VERDICT": "ASCENDANT OK" if versions == sorted(versions) else "DESORDRE"}
    names = [p.name for _, p in migs]
    out["T5_032_dash_absente"] = {
        "032_dans_liste": any("032" in n for n in names),
        "VERDICT": "032 tirets IGNOREE (confirme latent)" if not any(n.startswith("032") for n in names) else "032 presente",
    }
    ...
    try:
        ...
        out["T1_apply"] = {
            "version_finale": vfinal, "tables_apres": len(after),
    ...
    try:
        v2 = mm.apply()
        out["T2_idempotence"] = {"version_2e_run": v2, "VERDICT": "no-op OK" if v2 == out["T1_apply"].get("version_finale") else "DIVERGE"}
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        lost3 = {t: (before3[t], after3.get(t)) for t in before3
                 if isinstance(before3[t], int) and isinstance(after3.get(t), int) and after3[t] < before3[t]}
        out["T3_vieux_schema_replay"] = {
            "version_finale": v3, "tables_perdues_ou_reduites": lost3,
            "VERDICT": "self-healing IDEMPOTENT OK (aucune perte)" if not lost3 else "PERTE/ROUGE",
        }
    except Exception as e:  # noqa: BLE001
        out["T3_vieux_schema_replay"] = {"EXCEPTION": f"{type(e).__name__}: {e}", "VERDICT": "REPLAY NON-IDEMPOTENT (ROUGE)"}
    ...
    if OLD_DB.is_file():
        ...
        try:
            ...
            lost6 = {t: (before6[t], after6.get(t)) for t in before6
                     if isinstance(before6[t], int) and isinstance(after6.get(t), int) and after6[t] < before6[t]}
            out["T6_vrai_vieux_schema_v162"] = {
                "version_avant": v6_0, "version_apres": v6, "idempotent_run2": v6b == v6,
                "tables_avant": len(before6), "tables_apres": len(after6),
                "tables_perdues_ou_reduites": lost6,
                "VERDICT": "UPGRADE vieux schema OK (lossless + idempotent)"
                           if not lost6 and v6b == v6 else "PERTE/NON-IDEMPOTENT (ROUGE)",
            }
        except Exception as e:  # noqa: BLE001
